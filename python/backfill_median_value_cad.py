#!/usr/bin/env python3
"""
Backfill lowest_price_cad on physical_media rows.

Reads records where discogs_release_id IS NOT NULL and lowest_price_cad IS NULL,
calls the Discogs marketplace/stats API for each, and updates the row via the
Supabase REST API.

Usage:
  # Dry run (default) — prints what would be updated
  python backfill_lowest_price_cad.py

  # Apply changes
  python backfill_lowest_price_cad.py --apply

  # Limit to first N records (useful for testing)
  python backfill_lowest_price_cad.py --apply --limit 10
...
def to_cad(value: float, currency: str) -> float:
    ...
    if cache_key not in _fx_cache:
        ...
        _fx_cache[cache_key] = resp.json()['rates']['CAD']
    return round(value * _fx_cache[cache_key], 2)
...
def fetch_marketplace_value(release_id: int, consumer_key: str, consumer_secret: str) -> float | None:
    """Call Discogs marketplace/stats and return lowest_price converted to CAD, or None."""
    ...
    try:
        ...
        if lowest and lowest.get('value') is not None:
            return to_cad(float(lowest['value']), lowest.get('currency', 'USD'))
        return None
...
def update_record(supabase_url: str, service_key: str, record_id: str, value: float) -> bool:
    """PATCH lowest_price_cad on a single physical_media row."""
    headers = {
        'apikey': service_key,
        'Authorization': f'Bearer {service_key}',
    ...
    resp = requests.patch(url, headers=headers, params=params, json={'lowest_price_cad': value}, timeout=15)
    return resp.ok


def main():
    parser = argparse.ArgumentParser(description='Backfill lowest_price_cad from Discogs marketplace stats')
    ...
    print('Fetching records with missing lowest_price_cad…')
